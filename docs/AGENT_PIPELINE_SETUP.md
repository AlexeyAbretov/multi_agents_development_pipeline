...
Сейчас из коробки поднимаются **P0–P18 + UI**: оркестратор, deployer, очередь. Порты — [`pipeline/ports.env`](../pipeline/ports.env) (по умолчанию UI `http://127.0.0.1:3010/`). Локальная отладка оркестратора (VSCode / `npm start`) — §10.
...
4. После ответа аналитика: комментарий со статусом, моделью и токенами прогона, **отдельный комментарий с текстом плана**, снимается `in-analysis`, ставится `ready-for-dev` или `needs-human`.
...
Имена **точно** такие (иначе GitHub API постановки меток вернёт 404). Создать недостающие:
