...
| `pipeline/src/providers/CursorProvider/` | Промпт + issue/PR, комментарии issue (analyst — все; developer — последний план аналитика; tester — план + последний отчёт developer), `Agent.create` cloud (`fast=false` для composer/grok), `run.wait()`, `getUsage()`. Класс `CursorClient`. Промпт: `pipeline/prompts/<role>.md`. |
